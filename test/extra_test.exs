defmodule Test.DataSpecs.Loader.Extra do
  ...
  import DataSpecs.Loader.Extra, only: [type_loaders: 0]
  ...
  describe "extra loaders" do
    test "isodatetime" do
      ...
      assert {:ok, datetime} == DataSpecs.load(iso_datetime_string, {@types_module, :t_datetime}, type_loaders())
    end
    ...
    test "isodatetime error" do
      ...
      assert {:error, ["can't convert \"#{value}\" to a DateTime.t/0 (:invalid_format)"]} ==
               DataSpecs.load(value, {@types_module, :t_datetime}, type_loaders())
    ...
    test "isodate" do
      ...
      assert {:ok, date} == DataSpecs.load(iso_date_string, {@types_module, :t_date}, type_loaders())
    end
    ...
    test "isodate error" do
      ...
      assert {:error, ["can't convert \"#{value}\" to a Date.t/0 (:invalid_format)"]} ==
               DataSpecs.load(value, {@types_module, :t_date}, type_loaders())
    ...
    test "mapset" do
      assert {:ok, MapSet.new(1..3)} == DataSpecs.load(1..3, {@types_module, :t_mapset}, type_loaders())

      assert {:ok, MapSet.new(["1", :a, 1])} ==
               DataSpecs.load(["1", :a, 1], {@types_module, :t_mapset_1}, type_loaders())
    end
    ...
    test "mapset error" do
      ...
      assert {:error, ["can't convert #{value} to a MapSet.t/1, value not enumerable"]} ==
               DataSpecs.load(value, {@types_module, :t_mapset}, type_loaders())
